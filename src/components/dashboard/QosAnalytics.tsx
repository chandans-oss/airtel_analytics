        ...
        <div className="space-y-6 animate-in slide-in-from-right duration-500 pb-10 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col gap-1 px-1">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-[14px] font-black uppercase tracking-[0.2em] text-foreground/90">QoS Analytics</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="rounded-xl border border-border bg-card p-3 flex items-center justify-between shadow-sm relative overflow-hidden">
                ...
                <div className={cn("rounded-xl border border-border bg-card p-3 flex items-center justify-between shadow-sm cursor-pointer transition-all hover:scale-[1.02] active:scale-95 group min-h-[85px]", filter === 'Packet Drops' && "ring-2 ring-amber-500/50 border-amber-500/30 shadow-lg")} onClick={() => setFilter('Packet Drops')}>
                    <div>
            ...
            <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-xs font-black uppercase tracking-widest text-foreground">Policy & Config Diagnostics</h3>
                    </div>
                    <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider opacity-60">
                        Policy Compliance & Queue Diagnostic Breakdown
                    </span>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    {[
                        { label: 'MIB Limit', desc: '(SNMP polling throttled)', color: 'rose', issue: 'SNMP MIB Limit' },
                        { label: 'Auth Failure', desc: '(Credential mismatch)', color: 'orange', issue: 'Auth Failure' },
                        { label: 'CLI Error', desc: '(Config parse failure)', color: 'amber', issue: 'CLI Error' },
                        { label: 'Policy Skip', desc: '(Class-map missing)', color: 'blue', issue: 'Policy Skip' },
                        { label: 'Discovery Fail', desc: '(Interface unmapped)', color: 'purple', issue: 'Discovery Fail' }
                    ].map((item) => {
                        const issueData = QOS_DATA.filter(d => d.issue === item.issue);
                        ...
                        return (
                            ...
                            >
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        exportToCSV(issueData, `QoS_${item.label.replace(/\s+/g, '_')}_Issues`);
                                    }}
